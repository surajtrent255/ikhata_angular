...
export class MergeProductComponent {
  @Input() id!: number;
  @Input() toggleMergeProduct!: boolean;
  ...
  ngOnChanges() {
    if (this.toggleMergeProduct) {
      this.ngxSmartModuleService.getModal('mergeProduct').open();
  ...
  ngAfterViewInit() {
    this.CommonService.enableDragging('mergeProduct', 'mergeProductPopup');
  }
  ...
  Merge(form: any) {
    ...
    this.SplitProductService.Merge(this.SplitProductObj).subscribe((res) => {
      this.toastrService.success('stock merged ');
      this.getallstock(
        this.SplitProductObj.updatedProductId,
        this.SplitProductObj.companyId
      );
  ...
  destroyComp() {
    this.ngxSmartModuleService.getModal('mergeProduct').close();
